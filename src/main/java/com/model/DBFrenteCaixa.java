...
//Contem Todas as Opera„·es de banco de dados
public class DBFrenteCaixa {
	...
	public double getTabelaDia(Connection con, DefaultTableModel diaModel) {
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(new JFrame(), "N„o foi possivel obter Vendas");
		}
	...
	public void insertVendaApagada(Connection con, String motivo, JTable tableVendaDia) {
		...
		for (int i = 0; i < selectRows.length; i++) {
			...
			try {
				...
				JOptionPane.showMessageDialog(new JFrame(), "N„o Foi poss„vel Apagar");
			}
		}
	...
	public int setValoresMovimento(Connection con, Time time, double dinhe, double cart, double troco) {
		try {
			...
			e2.printStackTrace();
			JOptionPane.showMessageDialog(new JFrame(), "Valores Inv„lidos");
		}
		return 0;
	}
	...
	/**Obtem todas as recargas para impress„o
	 * @return */
