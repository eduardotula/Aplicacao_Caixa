...
public class ImportarEstoque extends JFrame{
	...
	private String[] columnNamesImpo = new String[] {"Chave", "Cod Barra", "Descri��o",
			"Quantidade","V.Venda"};
	...
	public ImportarEstoque(Connection con) {
		...
		addRowTableEstoqueImport(con, estomodel, "SELECT IDPROD, CODBARRA, DESCRICAO , QUANTIDADE, VLR_ULT_VENDA, PRECO_CUSTO FROM PRODUTOS");
		...
		btnEditarNome.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(row > -1) {
					...
					String novoNome = JOptionPane.showInputDialog("Insira um novo nome",(String)importModel.getValueAtStr(modelRow, 2));
					if(novoNome != null && novoNome.length() > 0) {
						if(novoNome.length()<=80) {
							
							importModel.setValueAt(novoNome, modelRow, 2);
							
						}else {JOptionPane.showMessageDialog(null, "Nome n�o pode exeder 80 caracteres");}
					}else {JOptionPane.showMessageDialog(null, "Nome n�o pode ser vazio");}
				}else {
		...
		btnAdicionar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					e2.printStackTrace();
					JOptionPane.showMessageDialog(null, "Quantidade ou Valor Inv�lido");
				}
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int i = 0;i < tam;i++) {
					...
					ObjetoProdutoImport prod = new ObjetoProdutoImport(cod, desc, quanti, preco, 0.0, chave);
						boolean v = UpdateItemBd(
								con, prod.getChave(), prod.getCodBa(),
								prod.getProd(), prod.getValorUltV(),
								prod.getQuanti(), prod.getValorCusto());
						
						boolean f = false;
						if(quanti == 0) {
							
							f = dbVendas.InsertAlterPreco(
									con, dbVendas.getValorProd(con, prod.getChave()),
									prod.getValorUltV(), dbFrente.getFuncioCaixaAtual(con),
									prod.getChave());
							
							}else {
							f = dbVendas.addProdEntradas(
									con, quanti,0.0,preco, prod.getChave(),
									LocalDate.now(), LocalTime.now(),
									dbFrente.getFuncioCaixaAtual(con), dbVendas.getDefaultFornecedor());
	...
	public static void refreshEstoque(Connection con) {
		...
		rowSorterEsto = new TableRowSorter<DefaultModels>(estomodel);
		tableEsto.setRowSorter(rowSorterEsto);
		addRowTableEstoqueImport(con, estomodel, "SELECT IDPROD, CODBARRA, DESCRICAO , QUANTIDADE, VLR_ULT_VENDA FROM PRODUTOS");
		tableEsto.getColumnModel().getColumn(0).setMinWidth(0);
		tableEsto.getColumnModel().getColumn(0).setMaxWidth(0);
		tableEsto.getColumnModel().getColumn(0).setWidth(0);
	...
	public boolean UpdateItemBd(Connection con, int chave, String codBarra, String descr, double valor, Integer quantidade, double valorC) {
