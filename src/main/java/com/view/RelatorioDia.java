...
public class RelatorioDia extends JFrame {
	...
	private DefaultModels model = new DefaultModels(
			new String[] { "ID", "Codigo ", "Descrição", "Quantidade", "Unitario", "Dinheiro", "Cartão", "Total",
					"Tipo", "Hora", "IDEstoque" },
			new boolean[] { false, false, false, false, false, false, false, false, false, false },
			new Class<?>[] { Integer.class, String.class, String.class, Integer.class, Double.class, Double.class,
					Double.class, Double.class, String.class, LocalTime.class, Integer.class });
	// Objetos Visuais
	...
	public RelatorioDia(Connection con) {
		...
		btnImprim.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				for (int i = 0; i < tableVendaDia.getRowCount(); i++) {
					...
					PrintRelatoriosProds prod = new PrintRelatoriosProds((String) tableVendaDia.getValueAt(modelRow, 1),
							(String) tableVendaDia.getValueAt(modelRow, 2), (int) tableVendaDia.getValueAt(modelRow, 3),
		...
		btnDelete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (respo == JOptionPane.OK_OPTION && selectRows.length > 0) {
						...
						for (int row : selectRows) {
							...
							double valorDinheiro = (double) tableVendaDia.getValueAt(modelRow, 5);
							double valorCartao = (double) tableVendaDia.getValueAt(modelRow, 6);
							double valorTot = (double) tableVendaDia.getValueAt(modelRow, 7);
							int quanti = (int) model.getValueAt(modelRow, 3);
							int id = (int) model.getValueAt(modelRow, 10);
							LocalTime horaVenda = (LocalTime) tableVendaDia.getValueAt(modelRow, 9);
							int idProd = (int) tableVendaDia.getValueAt(modelRow, 10);
							DBOperations.DmlSql(con, "INSERT INTO VENDAAPAGA VALUES (NULL, ?, ?,?,?,?,?,?,?,?,?);",
									quanti, valorDinheiro, valorCartao, valorTot, date, horaVenda, date, time, motivo,
									idProd);
							DBOperations.DmlSql(con, "UPDATE PRODUTOS SET QUANTIDADE = QUANTIDADE + ? WHERE IDPROD = ?",
									quanti, id);
						}
		...
		gl_bottomPanel.setHorizontalGroup(gl_bottomPanel.createParallelGroup(Alignment.TRAILING)
				.addGroup(gl_bottomPanel.createSequentialGroup().addContainerGap(409, Short.MAX_VALUE)
						.addComponent(lblSomaDinh).addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(txtSomaDinhe, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(ComponentPlacement.RELATED).addComponent(lblSomaCart)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(txtSomaCart, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(ComponentPlacement.RELATED).addComponent(lblValorTot)
						.addPreferredGap(ComponentPlacement.RELATED).addComponent(txtValorTot,
						...
						gl_bottomPanel.createParallelGroup(Alignment.LEADING)
								.addGroup(gl_bottomPanel.createSequentialGroup()
										.addGroup(gl_bottomPanel.createParallelGroup(Alignment.BASELINE)
												.addComponent(txtValorTot, GroupLayout.PREFERRED_SIZE,
														GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
												.addComponent(lblValorTot)
												.addComponent(txtSomaCart, GroupLayout.PREFERRED_SIZE,
														GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
												.addComponent(lblSomaCart)
												.addComponent(txtSomaDinhe, GroupLayout.PREFERRED_SIZE,
														GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
		...
		gl_topPanel.setHorizontalGroup(gl_topPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_topPanel.createSequentialGroup().addContainerGap().addComponent(btnDelete)
						.addPreferredGap(ComponentPlacement.RELATED, 574, Short.MAX_VALUE).addComponent(btnImprim)
						.addContainerGap()));
		gl_topPanel.setVerticalGroup(gl_topPanel.createParallelGroup(Alignment.TRAILING)
				.addGroup(gl_topPanel.createSequentialGroup().addContainerGap(GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
						.addGroup(gl_topPanel.createParallelGroup(Alignment.BASELINE).addComponent(btnDelete)
		...
		if (MainVenda.IdCaixa > 0) {
			try {
				DBOperations.appendAnyTable(con,
						"Select V.CODESTO,P.CODBARRA,P.DESCRICAO,V.QUANTI,V.VALORUNI,V.VALORDINHEIRO,V.VALORCARTAO,V.VALORTOT,V.TIPOPAGAMENTO,V.HORA,P.IDPROD FROM VENDAS V"
								+ " INNER JOIN PRODUTOS P ON V.IDPROD = P.IDPROD WHERE V.CONTROLECAIXA_IDCAIXA = ?",
						model, MainVenda.IdCaixa);
			} catch (SQLException e) {
				Alerts.showError("Falha ao atualizar tabela de vendas", "Erro");
				e.printStackTrace();
			}
		} else {
			valorTotColumn = 0.0;
